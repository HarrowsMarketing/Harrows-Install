...
interface Props {
  ...
  isAdmin: boolean // Clerk admin: installer is reassignable. Installer session: name shown read-only.
  onClose: () => void
  onSaved: (report: EodReport) => void
}

// Mirrors AddReportModal.tsx's fields, pre-filled from an existing report and saved via
// PATCH instead of POST. The server independently enforces who may edit what (own reports
// only for an installer session, any report for a Clerk admin) and blocks editing a report
// that's already been marked processed — this UI only needs to not offer the button when
// ReportDetailModal's canEdit says it isn't allowed.
export default function EditReportModal({ report, visibleFields, isAdmin, onClose, onSaved }: Props) {
  const [installers, setInstallers] = useState<Installer[]>([])
  const [installerId, setInstallerId] = useState(report.installer_id || '')
  const [jobSearch, setJobSearch] = useState('')
  const [jobResults, setJobResults] = useState<JobCard[]>([])
  const [job, setJob] = useState<Pick<JobCard, 'id' | 'job_number' | 'project_name' | 'address'> | null>(report.job)
  const [date, setDate] = useState(report.report_date)
  const [percentComplete, setPercentComplete] = useState(report.percent_complete)
  const [workDone, setWorkDone] = useState(report.work_done)
  const [workScheduledTomorrow, setWorkScheduledTomorrow] = useState(report.work_scheduled_tomorrow || '')
  const [products, setProducts] = useState(report.products || '')
  const [issues, setIssues] = useState(report.issues || '')
  const [solutions, setSolutions] = useState(report.solutions || '')
  ...
  const [photoPathnames, setPhotoPathnames] = useState<string[]>(
    (report.photos ?? []).filter(p => !p.is_internal).map(p => p.blob_pathname)
  )
  ...
  useEffect(() => {
    if (isAdmin) axios.get('/api/install/people').then(r => setInstallers(r.data.people)).catch(() => {})
  }, [isAdmin])
  ...
  const submit = async () => {
    ...
    setError('')
    try {
      const r = await axios.patch(`/api/install/reports/${report.id}`, {
        jobId: job?.id || null,
        ...(isAdmin ? { installerId } : {}),
        reportDate: date,
        percentComplete,
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <p className="text-sm font-bold text-gray-900">Edit report</p>
        ...
        <div className="px-5 py-4 space-y-4">
          {isAdmin ? (
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Installer</label>
              <p className="mt-2 text-sm font-medium text-gray-900 px-3 py-2">{report.installer?.name || '—'}</p>
            </div>
          )}
          ...
          {visibleFields.photos && (
            <div>
              ...
              <div className="mt-2">
                <PhotoUpload reportKey={report.id} pathnames={photoPathnames} onChange={setPhotoPathnames} />
              </div>
            </div>
          )}
          ...
          <div className="bg-red-50 border-2 border-red-300 rounded-xl p-4">
            ...
            <div className="mt-3">
              <PhotoUpload reportKey={`${report.id}-internal`} pathnames={internalPhotoPathnames} onChange={setInternalPhotoPathnames} />
            </div>
          </div>
